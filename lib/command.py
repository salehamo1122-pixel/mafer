...
async def send_welcome_message():
    ...
    try:
        ...
        admin_first_name = getattr(admin_user, "first_name", None) or "friend"
        text = (
            f"**❈ Welcome, {admin_first_name}! 👋**\n"
            "**❈ Self client started successfully.**\n"
            "**❈ Created by @saleh681.**\n"
...
async def sc(event):
    if event.sender_id == admin_user_id:
        if sp is None:
            await event.edit("**❈ Spotify is not configured.**")
            return
        query = event.raw_text[7:].strip()
        if not query:
            await event.edit("**❈ Please provide a track or artist after /gmusic.**")
            return
...
async def calculator(event):
    ...
    try:
        ...
        await event.edit("**❈ Invalid calculation expression.**")
...
async def read_qr_code(event):
    if event.is_reply and event.sender_id == admin_user_id:
        ...
        await event.edit("**❈ Please reply to a message and use /readqr to detect a QR code.**")
...
patterns_actions = {
    ...
    'farsi on': ('settings/mode.txt', 'Farsi', '**❈ Farsi mode enabled.**'),
    'fancy on': ('settings/mode.txt', 'Fancy', '**❈ Fancy mode enabled.**'),
    'circle on': ('settings/mode.txt', 'Circle', '**❈ Circle mode enabled.**')
}
