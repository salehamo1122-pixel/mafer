...
def apply_clock_style(text):
    try:
        with open('settings/clock_style.txt', 'r', encoding='utf-8') as f:
            style = f.read().strip()
    except Exception:
        style = 'classic'
    tables = {
    ...
    return text.translate(str.maketrans('0123456789', tables.get(style, tables['classic'])))
...
async def update_last_name():
    ...
    while True:
        ...
        if option_enabled:
            ...
            current_time_str = apply_clock_style(apply_font(current_time_str, mode))
